...
export const defaultSingerAudioPolicy=Object.freeze({microphone:'',microphone2:'',reverbBus:'',musicMax:100,microphoneMax:77,microphone2Max:77,reverbMax:60});
...
export function singerAudioKey(policy,control){
  const microphone=control==='microphone2'?policy?.microphone2:policy?.microphone;
  if(!Object.hasOwn(singerMicrophones,microphone))return null;
  if(control==='microphone'||control==='microphone2')return `fader:${microphone}`;
...
export function singerAudioSnapshot({policy,musicVolume,musicReady,acappella,mixerLive,mixer}){
  ...
  for(const control of ['microphone','microphone2','reverb']){
    const key=singerAudioKey(policy,control),raw=mixer?.parameters?.[key];
    const reason=!key?'audio_unbound':!mixer?.connected?'mixer_unavailable':!mixerLive||!mixer?.synced?'mixer_syncing':!Number.isInteger(raw)||raw<0||raw>127||mixer.pendingDetails?.[key]?'audio_readback_pending':null;
...
export function singerAudioWrite(policy,control,value){
  ...
  const write=qu16IntentToWrite(control==='microphone'||control==='microphone2'?{kind:'fader',target:control==='microphone2'?policy.microphone2:policy.microphone,value}:{kind:'send',target:policy.microphone,mix:policy.reverbBus,value});
...
export async function executeSingerAudio(operation,context){
  ...
  if(!['music','microphone','microphone2','reverb'].includes(control)||!Number.isInteger(value)||value<0||!Number.isFinite(policy[`${control}Max`])||value>policy[`${control}Max`])throw new Error('invalid_audio_value');
