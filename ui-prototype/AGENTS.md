...
2026-09-11 加特林绿色复验覆盖此前禁用记录：在 TT-00608 / Show 2024.12.28，仅对 Fixture 42/17636 应用 Colour 72/33214、亮度4%，保持12秒后恢复自动联动，操作员明确确认“是绿色”。PGM green 现映射 33214；红/橙/黄仍33207，青/蓝/紫仍33219。不得继续将绿色归并为蓝色，也不得据此推断其他色板已通过现场验收。

2026-09-10 用户确认：加特林默认跟随 PGM 视频主色与主导 Deck 音乐节拍。冷启动默认自动模式、联动开启、逐拍跟随；连接已绑定 Titan 并校验身份后自动工作，不要求再次点击启用。保留操作员本次运行内手动暂停联动的能力；A 组光束点缀仍独立布防。配置包导入仍只更新配置，不触发现场输出。
...
Runtime media milestone: on Windows, the desktop app stores large media under `D:/KINGCLUB-Media` when D: is available, honors `KING_MEDIA_ROOT` in every build, and falls back to `%APPDATA%/club.king.broadcast-control/media` only when needed. It owns the `videos`, `audio`, and `images` children of that root and rescans local video/audio folders while running. It also reads KuGou's configured `DownloadPath` as an external audio source and `LyricPath` as its offline lyrics cache; playable files remain in place, while encrypted-audio conversion artifacts go to the managed D: media root. Until the bundled mpv sidecar replaces it, real MP4 playback uses the WebView2 media element in PGM/output; PVW and the operator-side PGM monitor must stay muted to prevent duplicate venue audio. The output window alone may emit the video's own audio, controlled by the independent R1 video-audio toggle.
...
主唱调音由中控保存通道绑定及上限。仅允许专业麦克风 CH1/CH2 或 GS CH6 的单个通道推子，以及该通道到已确认混响 FX1/FX2 的发送量；默认未绑定。不得猜测现场通道或自动改 Gain、LR、Mute、PAFL、幻象电源和效果器类型。真机必须 End Sync 且收到参数回读才能成功；重连不重放。清唱只把电脑音乐降为 0，保留麦克风、混响和歌词时钟；退出恢复之前伴奏音量并服从新上限。本机手动改变伴奏总音量会退出清唱。

2026-09-10：“唱完续播原歌单”同时适用于直接在歌单选歌演唱和临时插播。直接选歌的 single 模式在真实 EOF 时，若开关开启，应从该 Deck 保存的来源歌单继续下一首，以原唱、原调恢复顺播；关闭仍单曲停止。只处理主唱绑定 Deck，不覆盖人工 repeat/sequence/shuffle 或另一 Deck。曲终读取最新开关，命令忙时延后处理 EOF，不能将事件标为已处理后丢失。

2026-09-10 静音开声保护：操作员要求的是暂停→播放、音乐0→有声/退出清唱时约400ms短渐入。音乐总音量设定保持不变，麦克风和混响不参与；正常两首歌的Auto-DJ仍使用原有双Deck交叉曲线，禁止另加开声渐入叠在交叉曲线上。渐入工作在独立Deck后台线程，平时不增加轮询。

2026-09-10 底部“设置”左侧新增“专业数字音频处理器”，用于 DP448 输出线路、酒吧平面喇叭位置/朝向和通道参数。复用现场SVG平面，提供 A–F / 1–8 网格供用户标注；不得伪造喇叭位置。历史仅确认 OUT1–2 舞台两只15寸全频和 OUT7–8 四只低音链路，单只路由未确认时保留候选而不自动分配。一个输出可关联多只喇叭，共用通道DSP参数；距离字段为现场测量，声程时间估算不等于应写入的补偿延时。当前DP448未连接，布局和离线参数草稿可本地保存/导入导出，真实参数显示未读取且禁止下发，不得把草稿伪称设备状态。

2026-09-10 后续实机接入：设备通过 COM3/115200 确认为 ID1 XTA DP448 V3.01。原生驱动只开放已验证的输出增益与延时，遵守设备联动组、连接 epoch、写前参数版本检查及写后完整输出块回读；OUT1/2 实测联动，不能称为独立单输出变化。AudioCore 与 KINGCLUB 不同时占用串口，退出处理器页面或通讯异常释放连接，不重放失败写入。布局/草稿继续仅本地保存；分频、EQ、限幅、极性和静音未完成协议核验前保持禁止原生下发。细节见 docs/integrations/DP448-PROTOCOL.md。
